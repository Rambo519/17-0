...
const OUTPUT = path.join(
  ...
  "public",
  "brand",
  "gold-football-17-emblem.png",
);

/** Measured field color in the master PNG (not THEME_COLOR). */
const NAVY = [2, 14, 44] as const;
...
function isBlackRim(r: number, g: number, b: number): boolean {
...
async function main(): Promise<void> {
  ...
  if (meta.hasAlpha) {
    console.log("Source has an alpha channel; still isolating the football badge.");
  } else {
    console.log("Source is opaque RGB; isolating the football by gold connectivity.");
  }
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (keep[i]) continue;
      const pi = i * 4;
      if (!isBlackRim(data[pi]!, data[pi + 1]!, data[pi + 2]!)) continue;
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      const g = data[pi + 1]!;
      const b = data[pi + 2]!;
      if (isGold(r, g, b) || isBlackRim(r, g, b)) continue;

      const dNavy = dist(r, g, b, NAVY);
      if (dNavy > 36) continue;
      const alpha = Math.max(0, Math.min(1, (dNavy - NAVY_LIMIT) / 22));
      if (alpha <= 0.05) {
        data[pi + 3] = 0;
        defringed += 1;
        continue;
      ...
      const ia = 1 - alpha;
      data[pi] = Math.max(0, Math.min(255, Math.round((r - ia * NAVY[0]) / alpha)));
      data[pi + 1] = Math.max(0, Math.min(255, Math.round((g - ia * NAVY[1]) / alpha)));
      data[pi + 2] = Math.max(0, Math.min(255, Math.round((b - ia * NAVY[2]) / alpha)));
  ...
  for (let y = cropTop; y < cropTop + cropHeight; y++) {
    for (let x = cropLeft; x < cropLeft + cropWidth; x++) {
      ...
      const g = data[pi + 1]!;
      const b = data[pi + 2]!;
      if (dist(r, g, b, NAVY) <= NAVY_LIMIT && !isBlackRim(r, g, b)) fringe += 1;
    }
  }
  ...
  if (fringeRate > 0.01 || fringe > 40) {
    throw new Error(
      `Navy cannot be removed cleanly (${fringe} fringe pixels, ${(fringeRate * 100).toFixed(2)}% of silhouette). Not writing.`,
    );
  }

  if (bw > w * 0.98 && bh > h * 0.98) {
    throw new Error("Badge crop still fills the rounded square — knockout failed. Not writing.");
  }
